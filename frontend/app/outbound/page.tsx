...
export default function OutboundPage() {
  ...
  const selectedItem = items.find((it) => it.id === Number(form.item_id));
  ...
  async function handleSubmit() {
    try { setSubmitting(true); setConfirming(false);
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <header className="flex items-center justify-between gap-3 mb-5">
        <div className="accent-bar accent-bar-danger"><h2 className="text-lg font-extrabold tracking-tight" style={{ color: "var(--hui-text)" }}>出库管理</h2><p className="text-xs mt-0.5 uppercase tracking-wider font-semibold" style={{ color: "var(--hui-text3)" }}>库存自动扣减</p></div>
        <button className="hui-btn hui-btn-solid hui-btn-sm" onClick={() => { setForm(f => ({ ...f, operator: username || "" })); setShowForm(true); }}><IconPlus size={16} />新 增</button>
      </header>
      ...
      {showForm && (
        <div className="hui-overlay" onClick={() => setShowForm(false)} role="dialog" aria-modal="true" aria-label="新增出库">
          <div className="hui-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handlePreSubmit} className="flex flex-col gap-3">
              ...
              <div className="flex justify-end gap-2 mt-2">
                ...
                <button className="hui-btn hui-btn-solid hui-btn-sm" type="submit" disabled={submitting}>确认出库</button>
              </div>
            </form>
          </div>
          ...
          {confirming && selectedItem && (
            <div className="hui-overlay" onClick={() => setConfirming(false)} style={{ zIndex: 60 }}>
              <div className="hui-dialog" style={{ maxWidth: 380 }} onClick={(e) => e.stopPropagation()}>
                <h3 className="hui-dialog-title" style={{ color: "var(--hui-warning)" }}>确认出库</h3>
                <div className="text-sm mb-4" style={{ color: "var(--hui-text2)" }}>
                  <p className="mb-1">耗材：<strong style={{ color: "var(--hui-text)" }}>{selectedItem.name}</strong></p>
                  <p className="mb-1">数量：<strong style={{ color: "var(--hui-danger)" }}>-{form.quantity} {selectedItem.unit}</strong></p>
                  <p className="mb-1">操作人：<strong>{form.operator}</strong></p>
                  <p className="text-xs mt-2 px-2 py-1 rounded" style={{ background: "var(--hui-warning-light)", color: "var(--hui-warning)" }}>
                    出库后库存将从 {selectedItem.current_stock} 减少至 {selectedItem.current_stock - Number(form.quantity || 0)} {selectedItem.unit}
                  </p>
                </div>
                <div className="flex justify-end gap-2">
                  <button className="hui-btn hui-btn-bordered hui-btn-sm" onClick={() => setConfirming(false)}>取消</button>
                  <button className="hui-btn hui-btn-danger hui-btn-sm" onClick={handleSubmit} disabled={submitting}>{submitting ? "提交中..." : "确认出库"}</button>
                </div>
              </div>
            </div>
          )}
